...
cancer_data=pd.read_csv('Cancer_clean.csv', sep=',')
...
# Set X and y values
X=cancer_data[cancer_data.columns.difference(['Diagnosis','ID_number'])]
...
print('train: {}% | test {}%'.format(round(len(y_train)/len(cancer_data),2),
                                                       round(len(y_test)/len(cancer_data),2)))
